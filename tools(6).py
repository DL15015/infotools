    ...
    for index in range(len(dic['data']['pools'])):
        ...
        if pool24H:
            for analytic in pool24H:
                print(float(analytic["txVolumeUSD"]))
...
def getPoolData(url):
    body = """
        query data{
          pools {
            ...
            _48H:analytics(where:{type:"1",timestamp_gte:%d,timestamp_lt:%d},orderBy:timestamp,orderDirection:desc){
              tvlUSD
              feesUSD
            }
    ...
    """ % (preDayTimeStamp,curDayTimeStamp,pre2DayTimeStamp,preDayTimeStamp,cur8HTimeStamp,preWeek8HTimeStamp)
    data = findData(url,body)
    print(body)
    dic = json.loads(data)
    poolDict = {}
    for i in range(len(dic["data"]["pools"])):
        ...
        _48HtxVolume, _48HFeesUSD, _48HFeeAPR = 0, 0, 0
        for analytics in pool48HList:
            txVolume = float(analytics['txVolumeUSD'])
            feesUSD = float(analytics['feesUSD'])
            _48HtxVolume += txVolume
            _48HFeesUSD += feesUSD

        pool["VolumeChange"] = getChange(_24HtxVolume, _48HtxVolume)

        if (tvl != 0) & (pool48HList != []):
            _48HTVL = float(pool48HList[0]["tvlUSD"])
            _48HFeeAPR = (((_48HtxVolume * fee ) / _48HTVL) * 365 * 100)
        ...
        pool["Fee24HChange"] = getChange(_24HFeesUSD,_48HFeesUSD)
        pool["FeeAPRChange"] = getChange(_24HFeeAPR,_48HFeeAPR)
    ...
    body = """
        query data{
          tokens{
            ...
            _14Days:analytics(where:{type:"1",timestamp_gt:%d},orderBy:timestamp,orderDirection:desc){
              txVolumeUSD
            }
          }
        }
    """% (preDayTimeStamp,curDayTimeStamp,pre2DayTimeStamp,preDayTimeStamp,cur8HTimeStamp,preWeek8HTimeStamp,pre2Week8HTimeStamp)
    data = findData(url,body)
    dic = json.loads(data)
    tokenDict = {}
    for i in range(len(dic["data"]["tokens"])):
        ...
        token48HList = token["_48H"]
        token7DaysList = token["_7Days"]
        token14DaysList = token["_14Days"]
        ...
        _48HtxVolume, _48HFeesUSD = 0, 0
        for analytics in token48HList:
            txVolume = float(analytics['txVolumeUSD'])
            feesUSD = float(analytics['feesUSD'])
            _48HtxVolume += txVolume
            _48HFeesUSD += feesUSD

        token["Fee48H"] = _48HFeesUSD
        token["VolumeChange"] = getChange(_24HtxVolume, _48HtxVolume)
        token["Fee24HChange"] = getChange(_24HFeesUSD, _48HFeesUSD)
        ...
        _14DayTxVolume = 0
        for analytics in token14DaysList:
